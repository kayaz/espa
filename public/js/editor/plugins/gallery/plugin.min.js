        ...
        win = editor.windowManager.open({
            ...
            body: {
                ...
                items: [
                    {
                        type: 'htmlpanel',
                        html: '<link href="'+tinyMCE.baseURL+'/plugins/gallery/css/bootstrap.css" rel="stylesheet" type="text/css" />\n' +
            ...
            success: function (result) {
                $.each(result, function(i, item) {
                    $('<div class="col-3">' +
                        ...
                        '<span class="dropdown-item" data-type="gallery">Galeria</span>' +
                        '<span class="dropdown-item" data-type="carousel">Karuzela</span>' +
                        '<span class="dropdown-item" data-type="slider">Slider</span>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                    ...
                    $( ".dropdown-item" ).on( "click", function() {
                        ...
                        tinymce.activeEditor.execCommand('mceInsertContent', false, '<p>[galeria='+type+']'+ item.id +'[/galeria]</p>');
